...
def load_data():
    try:
        # Load vehicles data (adjust sheet_name if needed)
        df_vehicles = pd.read_excel("Price_LIST_RMC_ACCESSORIES.xlsx", sheet_name="Vehicles")
        # Load optional sheets if available
        try:
            df_rmc = pd.read_excel("Price_LIST_RMC_ACCESSORIES.xlsx", sheet_name="RMC")
        except Exception:
            df_rmc = pd.DataFrame()
            
        try:
            df_acc = pd.read_excel("Price_LIST_RMC_ACCESSORIES.xlsx", sheet_name="Accessories")
        except Exception:
            df_acc = pd.DataFrame()
...
if not df_vehicles.empty:
    ...
    filtered_models = df_vehicles[df_vehicles["MY"] == selected_my]["Model"].dropna().unique()
    selected_model = st.sidebar.selectbox("Vehicle Model", filtered_models)

    # Filter Variant Codes based on Model & MY
    filtered_variants = df_vehicles[
        (df_vehicles["MY"] == selected_my) & (df_vehicles["Model"] == selected_model)
    ]["VariantCode"].dropna().unique()
    ...
    if not df_rmc.empty and "RMC_Option" in df_rmc.columns:
        selected_rmc = st.sidebar.selectbox("RMC Contract", ["None"] + list(df_rmc["RMC_Option"].unique()))
        if selected_rmc != "None":
            rmc_price = float(df_rmc[df_rmc["RMC_Option"] == selected_rmc]["Price"].values[0])

    acc_price = 0.0
    if not df_acc.empty and "Accessory" in df_acc.columns:
        selected_acc = st.sidebar.multiselect("Accessories", list(df_acc["Accessory"].unique()))
        if selected_acc:
            acc_price = float(df_acc[df_acc["Accessory"].isin(selected_acc)]["Price"].sum())
    ...
    # Fixed Charges (per calculation setup)
    ...
    # Total Down Payment & Balance Due (Loan Financed Amount)
